...
$$
The corresponding `R` object is

```{r}
lin_form_centre <- as.formula("y ~ X3 + X4 + trt*I(X1 - mean(X1)) + trt*I(X2 - mean(X2))")
```


```{r mimR_stc}
mimR_stc <- mimR(AC.IPD, BC.ALD, strategy = strategy_stc(formula = lin_form_centre))
mimR_stc
```
