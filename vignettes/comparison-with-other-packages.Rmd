...
m <- glm(wt82_71 ~ qsmk + sex + race + age + I(age^2) +
           as.factor(education) + smokeintensity + I(smokeintensity^2) + smokeyrs + I(smokeyrs^2) + as.factor(exercise) + as.factor(active) + wt71 + I(wt71^2), 
         data = nhefs_dat)
summary(m)
...
##TODO: what to do with factors or more than 2 levels when aggregating?
...
lin_form <-
  as.formula(wt82_71 ~ trt + sex + race + age + I(age^2) + as.factor(education) +
...
res <- outstandR(BC.ALD = nhefs_ald, AC.IPD = nhefs_ipd, 
