...
This general format of these data sets from the original analysis consist of the following.
...
#### Rearranging the orginal aggregate data

The `{outstandR}` package take a slightly different data format for the aggregate data. A _long_ format is needs which is more generalisable, flexible and appropriate for handling programmatically. Having variable names as columns (in a _wide_) format makes manipulation and retrieval more cumbersome and common packages such as `dplyr` and other `tidyverse` packages are designed for long data frames. To transform from one format to another, we have created a helper function.
