...
ci.Delta.AB <- hat.mean.Delta.AB + qnorm(ci_range)*sqrt(hat.var.Delta.AB)
```
...
```{r}
AC_hat_stc <- stc_stats(data = AC.IPD)

hat.mean.Delta.BC <- marginal_treatment_effect(BC.ALD)
hat.var.Delta.BC <- marginal_variance(BC.ALD)

hat.Delta.AB <- AC_hat_stc$mean - hat.mean.Delta.BC
hat.var.Delta.AB <- AC_hat_stc$var + hat.var.Delta.BC

# construct Wald-type normal distribution-based confidence interval
ci.Delta.AB <- hat.Delta.AB + qnorm(ci_range)*sqrt(hat.var.Delta.AB)
```
...
AC_maic_boot <- boot::boot(data=AC.IPD, statistic=gcomp_ml.boot, R=1000,
...
ppv <- gcomp_stan(formula = as.formula("y ~ X3 + X4 + trt*X1 + trt*X2"),
                  dat = AC.IPD)
...
hat.delta.AC <- qlogis(rowMeans(ppv$y.star.A)) - qlogis(rowMeans(ppv$y.star.C))
