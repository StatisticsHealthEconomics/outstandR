...
b_0 <- 0.6             # baseline intercept coefficient; offset
...
Similarly, simulate for the aggregate data but with the additional summarise step.
...
*marginal variance*, defined as (for large samples) Var(log RR),

$$
\frac{1}{n_B} + \frac{1}{n_C} + \frac{1}{N_B} + \frac{1}{N_C}
$$

and the *marginal treatment effect*, defined as the log-relative risk,

$$
\log\left( \frac{n_B/N_B}{n_C/N_C} \right) = \log(n_B N_C) - \log(n_C N_B)
$$
...
$$
y = X_1 + X_2 + \beta_t X_3 + \beta_t X_4
$$
...
```{r}
lin_form <- as.formula("y ~ X1 + X2 + trt*X3 + trt*X4")
```
...
$$
y = X_1 + X_2 + \beta_t(X_3 - \bar{X_3}) + \beta_t(X_4 - \bar{X_4})
$$
