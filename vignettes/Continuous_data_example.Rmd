...
```{r generate-data}
# Generate data
betas <- c(0.5, 0.5, 0.5, 0.5)  # coefficients for the logistic regression
mean_linpred <- AC.IPD[, c("X1","X2","X3","X4")]%*%betas

AC.IPD <- read.csv(here::here("raw-data", "AC_IPD.csv"))  # AC patient-level data
AC.IPD$y <- rnorm(nrow(AC.IPD), mean = mean_linpred, sd = 1)
