...
# use the covariate already created
AC.IPD <- read.csv(here::here("raw-data", "AC_IPD.csv"))  # AC patient-level data

# EM: X1, X2
# PV: X3, X4

# Define the coefficients for the model (including interaction terms)
betas <- c(0.5, 0.5, 0.5, 0.5, 0.3, 0.4)
betas_trt1 <- c(0.6, 0.4, 0.5, 0.7, 0.3, 0.4)

AC.IPD$y <- NA

# Create interaction terms for effect modifiers (X1, X2) and treatment (trt)
AC.IPD$X1_trt <- AC.IPD$X1 * AC.IPD$trt
AC.IPD$X2_trt <- AC.IPD$X2 * AC.IPD$trt

# treatment group 0
group_0 <- subset(AC.IPD, trt == 0)
mean_linpred_0 <- as.matrix(group_0[, c("X1", "X2", "X3", "X4", "X1_trt", "X2_trt")]) %*% betas
residuals_0 <- rnorm(nrow(group_0), mean = 0, sd = 1)
group_0$y <- mean_linpred_0 + residuals_0

# treatment group 1
group_1 <- subset(AC.IPD, trt == 1)
mean_linpred_1 <- as.matrix(group_1[, c("X1", "X2", "X3", "X4", "X1_trt", "X2_trt")]) %*% betas_trt1
residuals_1 <- rnorm(nrow(group_1), mean = 0, sd = 1)
group_1$y <- mean_linpred_1 + residuals_1

# Combine the groups back into the original data
AC.IPD <- rbind(group_0, group_1)
```

or can use the in-built `gen_data()` function.
...
meanX_AC <- 0.45       # mean of normally-distributed covariate in AC trial
...
BC.ILD <- gen_data(N, b_trt, b_X, b_EM, b_0,
                   meanX_BC, sdX, corX, allocation,
                   family = "normal")
...
out.C <- filter(BC.IPD, trt == 1) %>%
  summarise(y.B.sum = sum(y),
            y.B.bar = mean(y),
            y.B.sd = sd(y),
            N.B = n())

out.B <- filter(BC.IPD, trt == 0) %>%
...
-   `y.*.bar`: mean outcome
-   `y.*.sd`: standard deviation of outcome
-   `N.*`: sample size
...
<!-- ```{r outstandR_maic} -->
<!-- outstandR_maic <- outstandR(AC.IPD, BC.ALD, -->
...
<!-- ```{r outstandR_stc} -->
<!-- outstandR_stc <- outstandR(AC.IPD, BC.ALD, -->
...
<!-- outstandR_gcomp_ml <- outstandR(AC.IPD, BC.ALD, -->
...
<!-- ```{r outstandR_mim} -->
<!-- outstandR_mim <- outstandR(AC.IPD, BC.ALD, -->
