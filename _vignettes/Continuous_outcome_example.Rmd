...
This is the vignette for performing population adjustment methods with continuous data, in order to compare marginal treatment effects when there are cross-trial differences in effect modifiers and limited patient-level data. We will demonstrate how to apply MAIC, G-computation with ML, G-computation with Bayesian inference and multiple imputation marginalisation. The document structure follow the binary data example vignette which should be referred to for more details.
...
In the following we will implement for MAIC and G-computation methods to obtain the *marginal variance* and the *marginal treatment effect*.
...
```{r outstandR_gcomp_bayes, eval=FALSE}
...
```{r outstandR_gcomp_bayes_eval, echo=FALSE}
xx <- capture.output(
  outstandR_gcomp_bayes <-
    outstandR(ipd_trial, ald_trial,
              strategy = strategy_gcomp_bayes(
