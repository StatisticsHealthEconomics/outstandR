...
patient-level data. We will demonstrate how to apply MAIC, 
...
In the following we will implement for MAIC and G-computation
...
```{r lin_form}
lin_form <- list(
  outcome_model = as.formula("y ~ trt + PF_cont_1 + PF_cont_2 + EM_cont_1 + EM_cont_2"),
...
```{r outstandR_gcomp_bayes, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r outstandR_gcomp_bayes_eval, echo=FALSE, message=FALSE, warning=FALSE}
xx <- capture.output(
  outstandR_gcomp_bayes <-
    outstandR(ipd_trial, ald_trial,
              strategy = strategy_gcomp_bayes(
...
res_tab <- 
  data.frame(
    ...
    `MAIC` = unlist(outstandR_maic$results$contrasts$means),
    `Gcomp ML` = unlist(outstandR_gcomp_ml$results$contrasts$means),
    `Gcomp Bayes` = unlist(outstandR_gcomp_bayes$results$contrasts$means),
    `MIM` = unlist(outstandR_mim$results$contrasts$means)) |> 
  round(2)
...
res_tab_var <- 
  data.frame(
    ...
    `MAIC` = unlist(outstandR_maic$results$contrasts$variances),
    `Gcomp ML` = unlist(outstandR_gcomp_ml$results$contrasts$variances),
    `Gcomp Bayes` = unlist(outstandR_gcomp_bayes$results$contrasts$variances),
    `MIM` = unlist(outstandR_mim$results$contrasts$variances)) |> 
  round(2)
...
ggplot(aes(x = Estimate, y = id, col = type), data = plotdat) +
  ...
  scale_color_manual(values = c("d_naive" = "black", "d_true" = "red", 
                                "MAIC" = "#E69F00", "Gcomp ML" = "#56B4E9", 
                                "Gcomp Bayes" = "#009E73", "MIM" = "#F0E442"))
```
