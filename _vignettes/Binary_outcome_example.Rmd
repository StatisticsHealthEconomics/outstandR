...
Alternatively, we can compute this using `outstandR`'s built-in `calc_bucher_naive()` function:
...
res_tab_lrr <- 
  data.frame(
    ...
    `MAIC` = unlist(outstandR_maic_lrr$results$contrasts$means),
    `Gcomp ML` = unlist(outstandR_gcomp_ml_lrr$results$contrasts$means),
    `Gcomp Bayes` = unlist(outstandR_gcomp_bayes_lrr$results$contrasts$means),
    `MIM` = unlist(outstandR_mim_lrr$results$contrasts$means)) |> 
  round(2)
...
res_tab_var_lrr <- 
  data.frame(
    # d_true = c(NA, NA, NA),
    # d_naive = c(var.d.AB.naive, d_AC_naive$var_AC, var.d.BC),
    `MAIC` = unlist(outstandR_maic_lrr$results$contrasts$variances),
    `Gcomp ML` = unlist(outstandR_gcomp_ml_lrr$results$contrasts$variances),
    `Gcomp Bayes` = unlist(outstandR_gcomp_bayes_lrr$results$contrasts$variances),
    `MIM` = unlist(outstandR_mim_lrr$results$contrasts$variances)) |> 
  round(2)
