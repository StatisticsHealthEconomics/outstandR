...
- *Matching-Adjusted Indirect Comparison (MAIC)* is based on weighting,
  ...
  beyond the observed covariate space. It reweights the individual
  patient-level data (IPD) to match the aggregate characteristics of the
  comparator trial, thereby aligning the populations.
...
we form estimators $`\hat{\Delta}{BC(BC)}`$ and
$`\hat{\Delta}_{AC(AC)}`$ of the trial level (or marginal) relative
treatment effects. We shall assume that this is always represented as a
difference so, for example, for the risk ratio this is on the log scale.

``` math
\hat{\Delta}_{AC{(AC)}} = g(\bar{Y}_{C{(AC)}}) - g(\bar{Y}_{A{(AC)}})
```

and similarly for $`\hat{\Delta}_{BC(BC)}`$. If we assume that there is
no difference in effect modifiers between trials, then the estimator of
the relative treatment effect $`d_{AB(BC)}`$ is
