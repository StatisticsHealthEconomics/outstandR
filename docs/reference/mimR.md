...
This is the main, top-level wrapper for `{mimR}`. Methods taken from
(Remiro‐Azócar et al. 2022) .

## Usage

``` r
mimR(AC.IPD, BC.ALD, strategy, CI = 0.95, ...)
```
...
mimR_gcomp_bayes <- mimR(AC_IPD, BC_ALD, strategy = strategy_gcomp_bayes(lin_form))
#> 
...
#> Chain 2: 
#> Chain 2:  Elapsed Time: 0.505 seconds (Warm-up)
#> Chain 2:                0.564 seconds (Sampling)
#> Chain 2:                1.069 seconds (Total)
#> Chain 2: 
```
