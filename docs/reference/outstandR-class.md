...
-   absolute:
    ...
    -   `variances`: Variance-covariance matrix of the absolute
        outcomes.
...
-   model:
    ...
    -   `weights`, `ESS`: (MAIC only) The estimated weights and
        Effective Sample Size.

    -   `stan_args`: (Bayesian G-Comp, MIM) Arguments passed to Stan.

    -   `rho`: (G-Comp ML, MIM, Bayesian G-Comp) Correlation
        coefficient.

    -   `N`: (G-Comp ML, MIM, Bayesian G-Comp) Number of iterations.

    -   `nu`, `hats.v`, `M`: (MIM only) Imputation parameters and
        matrices.
