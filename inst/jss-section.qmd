...
The next thing is to define the formula which specifies the relationship between the covariates, treatments and outcome. For this example the outcome regression is:
...
Similarly, the STC, G-computation and MIM analyses are performed as follows.
...
The results across all adjustment strategies for the binary outcome are summarized in Figure~fig:summary-forest-plot by calling the following. 
...
where $N_t$ is the number of subjects, and $s_t^2$ is the observed standard deviation, for treatment arm $t$.
...
outstandR_maic <- outstandR(
  ...
  strategy = strategy_maic(
    formula = lin_form_mixedX,
    family = gaussian(link = "identity")
  )
)
```

If we interrogate the output we see the same list format. Differences to the first example are the model and scale components correspond to the continuous input data used.
...
Recalling that the log link is defined separately in the `outstandR()` function call, we shall use the same linear formula as for the binary outcome example, `lin_form_contX`.
...
The `scale` argument in `outstandR()` allows users to request these alternative summary statistics directly. For example, to obtain a risk difference instead of the default log-odds ratio for the binary outcome analysis (previously shown in Section 4), we specify `scale = "risk_difference"`.
...
There are scenarios where manual specification via the `marginal_params` argument is also necessary. One use case is as a sensitivity analysis, where a researcher wishes to assess the robustness of the relative treatment effect estimates to deviations in the target population's characteristics of the IPD trial.
