...
Similarly, the ALD has the corresponding covariates but not as separate columns (wide format) but in a long format with all covariate names in the `variable` column. Notice that the treatment column `trt` is NA for the covariate because we assume that the covariate distribution is balanced to be the same between treatment arms.
...
The next step is to define the formula, which specifies the relationship between the covariates, treatments and outcome. For this example, the outcome regression is
...
The STC, G-computation and MIM analyses are performed similarly but with the `outcome_model` specified instead of the `balance_model`.
...
Forest plots of the results across all adjustment strategies for the binary outcome are given in Figure X by calling the following. 
...
```{r load-cont-data}
# A vs C individual-level data
data(AC_IPD_contY_mixedX)
head(AC_IPD_contY_mixedX)

# B vs C aggregate-level data
data(BC_ALD_contY_mixedX)
print(BC_ALD_contY_mixedX)
```

The formats are the same as the previous binary example but now in the ALD the binary-valued covariates have `statistic` entry `prop`. Covariates `X1` and `X3` are continuous and `X2` and `X4` are binary.
...
Let $\hat{\mu}_B$ be the mean in the comparator arm and $\hat{\mu}_C$ be the mean in the reference arm. Then the mean treatment difference is simply:
...
where, for treatment $t$, $N_t$ and $s_t^2$ are the number of patients and the observed standard deviation, respectively.
...
The formats are the same as the binary outcome data example. Let $\lambda$ be the outcome event rate, then the equation for the outcome regression for the count data is
...
$$
\hat{\Delta}_{BC} = \ln(\hat{\mu}_C) - \ln(\hat{\mu}_B)
$$
...
$$
Table 4 gives the full list of potential relative effects for count outcome data.
...
By default, `outstandR` reports treatment effects on the scale corresponding to the regression model's link function. For instance, a logistic regression specified with family `binomial(link = "logit")` naturally yields estimates in log-odds ratios, while a linear regression yields mean differences. However, health economic models often require parameters on specific scales, such as risk differences (RD) or relative risks (RR), to facilitate interpretation or align with decision-analytic model inputs.
...
The `scale` argument in `outstandR()` allows users to request these alternative summary statistics directly. When converting between relative measures (e.g., odds ratios) and absolute measures (e.g., risk differences), the package automatically estimates the necessary baseline risk ($P_0$) from the reference treatment arm in the IPD study.
...
Figure 2 shows the histogram of MAIC weights.
...
Figure 3 shows the MCMC trace plots.
