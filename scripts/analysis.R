...
simulation.metrics <- pc[rep(seq_len(nrow(pc)), each=4), ]
...
ate.table <- pc[rep(seq_len(nrow(pc)), each=4*replicates), ]
ate.table["Method"] <- NA
ate.table["ATE"] <- NA
...
# change from 1-9 for different scenarios
i <- 1

scenario.ates <- subset(ate.table,
...
p1 <- ggplot(scenario.ates, aes(x=Method, y=ATE, fill=Method)) + 
  ...
  scale_x_discrete(limits=c("MAIC", "STC", "G-comp (ML)", "G-comp (Bayes)")) +
